...
- Frontend tests run with Vitest + Testing Library (`vitest run`) against `frontend/src/tests` and any `*.test.jsx`/`*.test.js` files colocated with components. The committed suites cover login, the auth and permission gates, hotel search, hotel details, room selection, the multi-step checkout (including the exact booking payload it POSTs) and the booking confirmation page. `src/tests/testUtils.jsx` renders a component inside the session context + toast provider with a seeded `user` and a stubbed `next/navigation` location (`pathname`/`search`/`params`), returning `router` spies to assert navigation against and a `getUser()` probe for reading the live session; `src/tests/fixtures.js` holds API-shaped fixtures.
