...
const DEFAULTS = {
  ...
  currency: 'USD',
  // The currency codes selectable anywhere in the app (the default-currency
  // picker here, and every form that records a price in its own currency --
  // room rates, tour packages). Admins extend this list from Settings rather
  // than typing a currency code freehand, so a typo can't create a booking or
  // rate that formatCurrency then can't render sensibly.
  available_currencies: ['USD', 'BDT', 'EUR', 'GBP', 'INR', 'AUD'],
  timezone: 'UTC',
