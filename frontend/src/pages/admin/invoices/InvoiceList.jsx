...
const COLUMNS = [
  { key: 'invoiceNumber', header: 'Invoice #', render: (row) => <Link to={`/admin/invoices/${row.id}`}>{row.invoiceNumber}</Link> },
  {
    key: 'booking',
    header: 'Booking #',
    render: (row) => (row.booking ? <Link to={`/admin/bookings/${row.booking.id}`}>{row.booking.bookingNumber}</Link> : '—'),
  },
