 ...
 * file under uploads/invoices/ and stores its absolute URL on invoice.pdfUrl
 * -- same convention as uploadService.storeFile, so a PDF link works from an
 * email client or any other API consumer, not just the browser tab it was
 * generated in; otherwise the buffer is only returned (no S3 upload here --
...
export async function getInvoicePdf(id) {
  ...
  if (env.fileStorageDriver === 'local') {
    ...
    const absoluteUrl = `${env.publicUrl}/${path.posix.join('uploads', 'invoices', fileName)}`;
    await invoiceRepository.updatePdfUrl(id, absoluteUrl);
    invoice.pdfUrl = absoluteUrl;
  }
