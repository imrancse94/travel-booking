...
export const env = {
   ...
   * the agency logo, and any API consumer that is not the web app. Also used
   * to build the verify-email/reset-password links a browser actually opens.
   *
   * Now that the API and the web app are the same Next.js process (no more
   * separate origins to CORS between), this is the one "what is our public
   * URL" setting -- there is no CORS_ORIGIN to derive it from any more.
   */
  publicUrl: (process.env.PUBLIC_URL || 'http://localhost:5173').trim().replace(/\/+$/, ''),
